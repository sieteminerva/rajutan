  ...
  protected template(typeKey: InputElementType, el: HTMLElement, payload?: any): void {
    ...
    switch (typeKey) {
      case "@field": {
        ...
        mainInputNode && mainInputNode.__outer ? el.appendChild(mainInputNode.__outer) : el.appendChild(mainInputNode);
        ...
        if ((targetType === "text" || targetType === "number") && (payload.config?.className === "loading")) {
      ...
      case "@field>file": {
        ...
        el.id = payload.id;
        (el as HTMLInputElement).name = cfg.name ? String(cfg.name) : payload.id;
        ...
        if (typeKey === "@field>input") {
          (el as HTMLInputElement).type = cfg.type || "text";
        }
        ...
        if ((typeKey === "@field>radio")) {
          (el as HTMLInputElement).type = cfg.type;
          (el as HTMLInputElement).checked = true;
        }

        if (typeKey === "@field>select") {
          if (cfg.config?.className === "loading") el.classList.add("loading");
