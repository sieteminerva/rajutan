...
export type DerivedTheme = {
  ...
  /** Non-color values (gradients, shadows) per mode — painted inline for the
   *  active mode, since `light-dark()` only accepts `<color>`. */
  variants: { light: Record<string, string>; dark: Record<string, string> };
};
...
export class ColorThemeEngine {
  ...
  static colorSchema(
    ...
    // 🎨 SINGLE-MODE SHAPE — this map holds ONE mode's plain values. The zip
    // into `light-dark()` pairs happens in `splitTokens()` (via deriveTheme /
    // toCssText), because `light-dark()` only accepts `<color>`: gradients
    // and shadow lists ride `variants` instead — `light-dark(gradient, …)`
    // would die at computed-value time → "only some tokens apply".
    return {
      tokens,
      preview,
  ...
  /**
   * 🎨 COLOR GUARD — is this value legal inside `light-dark()` (i.e. a
   * `<color>`)? Gradients contain `gradient(`; shadow lists contain lengths
   * (`px`) — plain colors (`color-mix()`, `hsl()`, hex, `var()` refs) never do.
   */
  static isColorToken(value: string): boolean {
    const v = value.trim().toLowerCase();
    if (v.includes("gradient(") || v.includes("px")) return false;
    return true;
  }

  /**
   * 🔀 SPLIT — merge two single-mode token maps into ONE paintable map plus
   * both non-color branches. Differing COLORS become `light-dark(light, dark)`
   * (identical ones stay plain); NON-colors (gradients, shadow lists — illegal
   * in `light-dark()`) land in `tokens` as the CURRENT mode's plain value and
   * in `variants` per branch, so the mode toggle repaints them while colors
   * flip via `color-scheme` alone.
   */
  static splitTokens(
    light: Record<string, string>,
    dark: Record<string, string>,
    mode: ThemeMode,
  ): { tokens: Record<string, string>; variants: { light: Record<string, string>; dark: Record<string, string> } } {
    ...
    for (const key of keys) {
      const l = light[key] ?? dark[key];
      const d = dark[key] ?? light[key];
      if (l === d) { tokens[key] = l; continue; }
      if (ColorThemeEngine.isColorToken(l) && ColorThemeEngine.isColorToken(d)) {
        tokens[key] = `light-dark(${l}, ${d})`;
        continue;
      }
      lightVariants[key] = l;
      darkVariants[key] = d;
      tokens[key] = mode === "dark" ? d : l;
    }
    return { tokens, variants: { light: lightVariants, dark: darkVariants } };
  }

  /**
   * 🔀 ZIP — merge two single-mode token maps into ONE map where differing
   * values become `light-dark(light, dark)` (identical ones stay plain). This
   * is THE single source of the zip format: `toCssText` exports it and
   * `#paintTokens` paints it inline — one injection serves both branches, and
   * a runtime mode flip is a pure `color-scheme` write instead of re-injecting
   * a different value set.
   */
  static zipTokens(light: Record<string, string>, dark: Record<string, string>): Record<string, string> {
    const keys = [...Object.keys(light), ...Object.keys(dark).filter((k) => !(k in light))];
    const zipped: Record<string, string> = {};
    for (const key of keys) {
      const l = light[key] ?? dark[key];
      const d = dark[key] ?? light[key];
      // 📎 Identical across modes → plain declaration; differing → zipped.
      zipped[key] = l === d ? l : `light-dark(${l}, ${d})`;
    }
    return zipped;
  }

  /**
   * Serialize BOTH modes as ONE `:root` block of `light-dark(light, dark)`
   * pairs — the variables.css format. One paste updates both branches (single
   * token copy), and the static `color-scheme` pin rules below `:root` in
   * that file stay untouched.
   */
  static toCssText(light: Record<string, string>, dark: Record<string, string>, bases: BaseColors): string {
    const head =
      `/* Generated by Rajutan Color Theme Generator — light-dark pairs\n` +
      `   bases: primary ${bases.primary} · accent ${bases.accent}\n` +
      `   every other token is derived from those two with color-mix()\n` +
      `   paste over :root in variables.css (the color-scheme pins stay below) */\n`;
    // 🔀 Non-colors can't zip — they ship as the light value in `:root` plus a
    // small dark branch (explicit pin + OS-auto), so one paste still carries
    // both worlds with no full-block duplication.
    const split = ColorThemeEngine.splitTokens(light, dark, "light");
    const body = Object.entries(split.tokens)
      .map(([k, v]) => `  ${k}: ${v};`)
      .join("\n");
    const darkBranch = Object.entries(split.variants.dark)
      .map(([k, v]) => `  ${k}: ${v};`)
      .join("\n");
    const darkBlock = darkBranch
